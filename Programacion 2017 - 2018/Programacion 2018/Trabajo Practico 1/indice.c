...
{
    int resultado = eliminardelista(&(pi)->l, reg);
    set_nodo_inicial(&(pi->ite), (pi->l));
    return resultado;
}
...
int ind_buscar(T_indice *pi, t_reg_indice *reg)
{
    ...
    while(ind_fin(pi) == 0) // Voy avanzando con el iterador por los registros de la lista.
    {
...
int ind_grabar(T_indice *pi, const char* path)
{
    ...
    while(!ind_fin(pi)) // Mientras no este en el final voy leyendo los registros.
    {
...
int ind_nro(T_indice *pi)
{
    ...
    get_reg_inicial(&(pi->ite), &registro); // Obtengo el primer registro de la lista.
    while(!ind_fin(pi)) // Mientras no este en el final voy leyendo los registros.
        get_reg_siguiente(&(pi->ite), &registro);
    int resp = registro.nro_reg;
    return (resp++);
}
