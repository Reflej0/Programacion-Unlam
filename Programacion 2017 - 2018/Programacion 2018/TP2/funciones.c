...
//Devuelve por vec_errores los errores especificos.
//Devuelve 1 si el alumno es correcto.
int validar_alumno(t_alumno const *alumno, tFecha const *f_actual, int* vec_errores)
{
    ...
    return ((vec_errores[0]+vec_errores[1]+vec_errores[2]+vec_errores[3]+vec_errores[4]+vec_errores[5]+vec_errores[6]+vec_errores[7]) == 8) ? 1 : 0;
}
...
void procesar_archivo_alumnos_txt(char* dir, tFecha* fact)
{
    ...
    if(!arch_txt) exit(1);
    FILE* arch_bin = fopen("alumnos_correctos.dat", "wb");
    if(!arch_bin) exit(1);
    FILE* arch_errores = fopen("alumnos_incorrectos.txt", "wt");
    if(!arch_bin) exit(1);
    t_alumno alu;
    fscanf(arch_txt, "%ld|%[^|]|%d/%d/%d|%c|%d/%d/%d|%[^|]|%d|%d/%d/%d|%c|%d/%d/%d", &alu.dni, alu.ayn, &alu.fnac.dia, &alu.fnac.mes,
    &alu.fnac.anio, &alu.sexo, &alu.fing.dia, &alu.fing.mes, &alu.fing.anio, alu.carrera, &alu.mataprobadas,
    &alu.ultmat.dia, &alu.ultmat.mes, &alu.ultmat.anio, &alu.estado, &alu.fbaja.dia, &alu.fbaja.mes, &alu.fbaja.anio);
    while(!feof(arch_txt))
    {
        int vector_errores[8] = {0};
        if(validar_alumno(&alu, fact, vector_errores))
            escribir_alumno_correcto(arch_bin, &alu);
        else
            escribir_alumno_incorrecto(arch_errores, &alu, vector_errores, CAMPOS);
        fscanf(arch_txt, "\n%ld|%[^|]|%d/%d/%d|%c|%d/%d/%d|%[^|]|%d|%d/%d/%d|%c|%d/%d/%d", &alu.dni, alu.ayn, &alu.fnac.dia, &alu.fnac.mes,
        &alu.fnac.anio, &alu.sexo, &alu.fing.dia, &alu.fing.mes, &alu.fing.anio, alu.carrera, &alu.mataprobadas,
        &alu.ultmat.dia, &alu.ultmat.mes, &alu.ultmat.anio, &alu.estado, &alu.fbaja.dia, &alu.fbaja.mes, &alu.fbaja.anio);
    }
    fclose(arch_txt);
...
{
    int i;
    fprintf(arch, "%ld|%s|%d/%d/%d|%c|%d/%d/%d|%s|%d|%d/%d/%d|%c|%d/%d/%d\n", alu->dni, alu->ayn, alu->fnac.dia, alu->fnac.mes,
        alu->fnac.anio, alu->sexo, alu->fing.dia, alu->fing.mes, alu->fing.anio, alu->carrera, alu->mataprobadas,
        alu->ultmat.dia, alu->ultmat.mes, alu->ultmat.anio, alu->estado, alu->fbaja.dia, alu->fbaja.mes, alu->fbaja.anio);
    for(i=0; i<tam; i++)
    {
        if(i==0 && *vector_errores == 0)
            fprintf(arch, "El dni es invalido\n");
        else if(i==1 && *vector_errores == 0)
            fprintf(arch, "La fecha de nac. es invalida\n");
        else if(i==2 && *vector_errores == 0)
            fprintf(arch, "El sexo es invalido\n");
        else if(i==3 && *vector_errores == 0)
            fprintf(arch, "La fecha de ingreso es invalida\n");
        else if(i==4 && *vector_errores == 0)
            fprintf(arch, "La carrera es invalida\n");
        else if(i==5 && *vector_errores == 0)
            fprintf(arch, "La fecha de apr. ult. mat es invalida\n");
        else if(i==6 && *vector_errores == 0)
            fprintf(arch, "El estado es invalido\n");
        else if(i==7 && *vector_errores == 0)
            fprintf(arch, "La fecha de baja es invalida\n");
        vector_errores++;
    }
}
