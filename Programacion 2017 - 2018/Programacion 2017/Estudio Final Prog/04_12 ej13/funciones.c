...
void Actualizar_Sueldo(FILE *f_empleados, FILE *f_estudiantes)
{
    ...
    {
        int comp = emp.dni - est.dni;
        //SI EL EMPLEADO Y EL ESTUDIANTE SON EL MISMO. Y ADEMAS EL PROMEDIO ES MAYOR a 7.
        if(comp == 0 && est.promedio > 7)
        {
            emp.sueldo*= 1.0728; // LE AUMENTO EL SUELDO.
            fseek(f_empleados, -sizeof(Empleado), SEEK_CUR); //VUELVO PARA ATRAS.
            fwrite(&emp, sizeof(Empleado), 1, f_empleados); // ACTUALIZO.
            //AVANZO AMBOS.
            fread(&emp, sizeof(Empleado), 1, f_empleados);
            fread(&est, sizeof(Estudiante), 1, f_estudiantes);
