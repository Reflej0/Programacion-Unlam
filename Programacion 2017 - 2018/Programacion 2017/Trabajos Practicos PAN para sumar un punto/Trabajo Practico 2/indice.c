...
{
    char directorio[100] = ""; //Variable que contendra EJ: C:\Carpeta\alumnos.idx
    strcpy(directorio, path); //Empiezo copiando el directorio EJ: C:\Carpeta\*
    strcat(directorio, "alumnos.idx"); // Al final agrego el alumnos.idx
...
int ind_grabar(T_indice *pi, const char* path)
{
    ...
    strcpy(directorio, path); //Empiezo copiando el directorio EJ: C:\Carpeta\*
    strcat(directorio, "alumnos.idx"); // Al final agrego el alumnos.idx
    FILE* arch = fopen(directorio, "wb"); // Abro el archivo.
