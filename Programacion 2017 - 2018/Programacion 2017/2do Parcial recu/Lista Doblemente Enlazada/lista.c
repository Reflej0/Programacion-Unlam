...
int eliminar_valor_lista(T_lista* pl, T_dato* d, T_cmp cmp)
{
    ...
    if(elim==*pl) *pl=elim->anterior;

    else *pl=elim->siguiente;

    if(elim->anterior) elim->anterior->siguiente=elim->siguiente;

    if(elim->siguiente) elim->siguiente->anterior=elim->anterior;
