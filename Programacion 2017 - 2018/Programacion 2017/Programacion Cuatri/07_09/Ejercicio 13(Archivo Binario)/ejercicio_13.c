...
int guardar_empleados(Empleado *Empleados, int tam)
{
    ...
    fwrite(Empleados, sizeof(Empleado), tam, f_empleados);
/*
    while(i<tam)
    {
        fwrite(&Empleados[i], sizeof(Empleado), tam, f_empleados);
        i++;
    }
*/
    fclose(f_empleados);
...
{
    rewind(bin); //Esto es para que me posicione arriba de todo en el archivo, osea en el primer registro.
    rewind(txt); //Esto es para que me posicione arriba de todo en el archivo, osea en el primer registro.
    Empleado emp;
    fread(&emp, sizeof(Empleado), 1, bin);
    while(!feof(bin))
    {
        fprintf(txt, "%08d%-20s%-20s%09.2f\n", emp.dni, emp.nombre, emp.apellido, emp.sueldo);
        fread(&emp, sizeof(Empleado), 1, bin);
    }
